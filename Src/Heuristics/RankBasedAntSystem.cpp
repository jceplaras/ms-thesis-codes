...
void updatePheromonesElitist(std::vector<double> & pheromoneLevelCity,std::vector<Solution> & ants,Solution & eliteSolution,double eliteSolutionBonus) {
...
void updatePheromonesRankBased(std::vector<double> & pheromoneLevelCity,std::vector<Solution> & ants,Solution & eliteSolution,double eliteSolutionBonus,int rankBasedCount) {
    std::vector<double> pheromoneIncreaseCity(pheromoneLevelCity.size(),0);
...
int main(int argc, char ** argv) {
    ...
    try {
        TCLAP::CmdLine cmd("Rank Based Ant System", ' ', "1.0");
        ...
        TCLAP::ValueArg<double> eliteSolutionBonusArgs("E","eliteSolutionBonus","Elitist Pheromone Bonus",true,4,"double",cmd);
        TCLAP::ValueArg<int> rankBasedCountArgs("c","rankBasedCount","Number of ranked ants",true,1,"int",cmd);
        cmd.parse(argc,argv);
        ...
        eliteSolutionBonus = eliteSolutionBonusArgs.getValue();
        rankBasedCount = rankBasedCountArgs.getValue();

        if(rankBasedCount > numberOfAnts) {
            std::cerr << "Number of ranked ants cannot be greater than number of ants" << std::endl;
            return 1;
        }
...
*/
        ...
        //update pheromones
        updatePheromonesRankBased(pheromoneLevelCity,ants,eliteAnts[0],eliteSolutionBonus,rankBasedCount);
