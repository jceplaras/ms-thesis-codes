...
int selectCityByExploitation(std::vector<double> & probabilitySelectCityNumerator) {
    int max = 0;
    int candidate = -1;
    FORN(i,probabilitySelectCityNumerator.size()) {
        if(probabilitySelectCityNumerator[i] > max) {
            max = probabilitySelectCityNumerator[i];
            candidate = i;
        }
    }
    return candidate;
...
void updatePheromonesAntColonySystem(std::vector<double> & pheromoneLevelCity,Solution & eliteAnt) {
    std::vector<double> pheromoneIncreaseCity(pheromoneLevelCity.size(),0);

    FORN(j,eliteAnt.getCurrentFacilityCount()) {
        int cityNumber = eliteAnt.getFacility(j);

        pheromoneIncreaseCity[cityNumber] += 1/(double)eliteAnt.getScore();
    }
...
int main(int argc, char ** argv) {
    ...
    try {
        ...
        TCLAP::ValueArg<double> exploitationRateArgs("x","exploitationRate","Percentage of selecting exploitation as facility selector",true,0.9,"double [0,1]",cmd);
        ...
        exploitationRate = exploitationRateArgs.getValue();
    ...
    //storage of locally inserted facilities
    std::vector<double> insertedFacilitiesForIteration(numberOfAnts);
    ...
    FORN(iterationNumber,maxIteration) {
        ...
        FORN(currentFacilityCount,numberOfFacilities) {
            FORN(currentAnt,numberOfAnts) {
                ...
                int candidate;
                do{
                    double facilitySelectionRule = probabilityGenerator(generator);

                    if(facilitySelectionRule <= exploitationRate) {
                ...
                insertedFacilitiesForIteration[currentAnt] = candidate;

            }

            //local pheromone update
            FORN(currentAnt,numberOfAnts) {
                pheromoneLevelCity[currentAnt] = (1-pheromoneEvaporationRate)*pheromoneLevelCity[currentAnt] + pheromoneEvaporationRate*pheromoneInitialValue;
            }
        ...
        //update pheromones
        updatePheromonesAntColonySystem(pheromoneLevelCity,eliteAnts[0]);
