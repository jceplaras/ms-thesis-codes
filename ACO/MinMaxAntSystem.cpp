...
void evaporatePheromonesMinMax(std::vector<double> & pheromoneLevelCity,double pheromoneEvaporationRate,double pheromoneMinimumValue) {
   FORN(i,pheromoneLevelCity.size()) 
       pheromoneLevelCity[i] = std::min(pheromoneLevelCity[i]*(1-pheromoneEvaporationRate),pheromoneMinimumValue);
}
...
int main(int argc, char ** argv) {
    ...
    FORN(iterationNumber,maxIteration) {
        ...
        if(isResetEnabled) {
                ...
                if(differenceBestWorstSolution <= differenceThreshold*numberOfFacilities || eliteAnts[0].getScore() == ants[numberOfAnts-1].getScore())
                    resetCounter++;
